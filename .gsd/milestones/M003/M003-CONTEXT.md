...
- **CLI auto-update:** Last prompt in `driftless init` asks "Would you like to enable auto-updates?" (default: yes). Stored as `autoUpdate: true` in `.driftless.json`. On every CLI launch, if enabled, check npm registry for a newer version and install it before running the command. Adds ~1-2s on cold runs. Must handle: network failures (skip silently, don't block), major version jumps (warn but still update), `npx` vs global install (detect and use the right update mechanism). **Package manager detection:** On update, detect which package manager originally installed driftless (npm, pnpm, yarn, bun) and use the correct global install command — e.g. `pnpm i -g driftless@latest`, `npm i -g driftless@latest`, `yarn global add driftless@latest`, `bun i -g driftless@latest`. Detection strategy: check `process.env.npm_config_user_agent` (set by all major package managers at install time and persisted in the npm registry metadata), or walk up from the resolved binary path to find which package manager's global directory it lives in. Store the detected package manager in `.driftless.json` as `packageManager: "pnpm"` during init so subsequent updates don't need to re-detect.
