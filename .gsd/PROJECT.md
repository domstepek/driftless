...
M001–M003 complete. v1.0.0 released to npm — both `@driftless-ai/core@1.0.0` and `@driftless-ai/cli@1.0.0` published and installable from the live registry. CI/CD pipeline in place: `.github/workflows/ci.yml` gates PRs with check+test+build, `.github/workflows/release.yml` automates npm publish with provenance + GitHub Release on `v*` tag push. GitHub repo has professional OSS presence: README with badges, MIT LICENSE, CONTRIBUTING.md, CODE_OF_CONDUCT.md, SECURITY.md, YAML form issue templates, PR template, FUNDING.yml, 9 topics, homepage URL, branch protection on main (PR review + CI required). CLI auto-update: checks npm registry on launch when `autoUpdate: true`, detects package manager, auto-installs; npx users get notification; network/CI failures skip silently. The CLI (`npx @driftless-ai/cli@latest init`) runs an interactive wizard that auto-detects test frameworks, prompts for configuration (including auto-update preference), spawns Claude Code in headless mode to generate training docs from e2e tests, installs parameterized skill files, scaffolds GitHub Actions workflow files based on selected capabilities, and writes `.driftless.json` config. Three doc framework adapters (plain-md, fumadocs MDX, docusaurus MDX). Fail-safe init with FileTransaction rollback, structured debug logging, and `--dry-run` preview. Two workflow templates for PR-triggered automation via `claude-code-action@v1`. Full capability matrix. 268 tests across 14 files. 28 of 36 requirements validated. M004 complete: polished editorial landing page and fumadocs docs site deployed at `driftless-six.vercel.app` with 5 docs pages (Quick Start, init walkthrough, GitHub Actions, configuration, troubleshooting). X/Twitter launch playbook written to `~/Desktop/driftless/m004-launch-playbook.md` with 18 posting sessions, Mermaid timeline, engagement strategy. Next: M005 (Business Infrastructure + Platform Vision).
