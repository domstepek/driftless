...
M001–M005 complete. v1.0.0 released to npm — both `@driftless-ai/core@1.0.0` and `@driftless-ai/cli@1.0.0` published and installable from the live registry. CI/CD pipeline in place: `.github/workflows/ci.yml` gates PRs with check+test+build, `.github/workflows/release.yml` automates npm publish with provenance + GitHub Release on `v*` tag push. GitHub repo has professional OSS presence: README with badges, MIT LICENSE, CONTRIBUTING.md, CODE_OF_CONDUCT.md, SECURITY.md, YAML form issue templates, PR template, FUNDING.yml, 9 topics, homepage URL, branch protection on main (PR review + CI required). CLI auto-update: checks npm registry on launch when `autoUpdate: true`, detects package manager, auto-installs; npx users get notification; network/CI failures skip silently. The CLI (`npx @driftless-ai/cli@latest init`) runs an interactive wizard that auto-detects test frameworks, prompts for configuration (including auto-update preference), spawns Claude Code in headless mode to generate training docs from e2e tests, installs parameterized skill files, scaffolds GitHub Actions workflow files based on selected capabilities, and writes `.driftless.json` config. Three doc framework adapters (plain-md, fumadocs MDX, docusaurus MDX). Fail-safe init with FileTransaction rollback, structured debug logging, and `--dry-run` preview. Two workflow templates for PR-triggered automation via `claude-code-action@v1`. Full capability matrix. 268 tests across 14 files. 32 of 36 requirements validated. Polished editorial landing page and fumadocs docs site deployed at `driftless-six.vercel.app` with 5 docs pages (Quick Start, init walkthrough, GitHub Actions, configuration, troubleshooting), dynamic OG image, and full Twitter Card meta tags. X/Twitter launch playbook at `~/Desktop/driftless/m004-launch-playbook.md` with 18 posting sessions, Mermaid timeline, engagement strategy. M005 complete: ten business planning documents at `~/Desktop/driftless/` — vision strategy, executive summary, business structure (LLC formation), product strategy, PRD, pricing model (3 revenue scenarios), payment infrastructure (Lemon Squeezy), GTM plan, operations playbook, and pitch deck outline. All 10 docs share canonical ICP, phase timeline, and feature labels with grep-verified cross-document consistency. Private repo `domstepek/driftless-pro` scaffolded with proprietary license. Architecture decisions D077–D079 formalized (payment provider, Pro license, repo organization). Next: M006 (Brand Identity + Landing Page Revamp).
...
- [ ] M006: Brand Identity + Landing Page Revamp — Full visual rebrand (brand identity doc + design system), spinning ASCII mesa hero component, landing page rebuild modeled on sutera.ch brutalist-technical-white aesthetic with geological amber accent
